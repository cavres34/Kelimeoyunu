...
if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    ...
    #sıl octopus bot
    bot_adi = "gold"
    bot_token = "6279713520:AAFhjLsRr7AinibICUsIgshTqdkcBaZVHFQ"
    bot = AsyncTeleBot(bot_token, parse_mode="html")
...
async def log_gonder(**kwargs):
    ...
    try:
        await bot.send_message(-1001937961971, f"""
<b> ~~ 📢 Log Kaydı ~~</b>
...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001937961971, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="🇹🇷 ʙᴇɴɪ ɢʀᴜʙᴀ ᴇᴋʟᴇ 🇹🇷", url="https://t.me/goldgamebot?startgroup=a")
            callback_button2 = types.InlineKeyboardButton(text="⚙️ ʀᴇsᴍɪ ᴋᴀɴᴀʟ ⚙️", url="https://t.me/goldsohbettr")
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001937961971, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
